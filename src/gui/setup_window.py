...
# 이 클래스들은 FluentWindow와 무관하므로 외부에 있어도 안전합니다.
SUPPORTED_LANGUAGES = {
...
class TranslationItem(QWidget):
    # (이전과 동일한 TranslationItem 클래스 코드)
...
# --- 핵심: 팩토리 함수 ---
def create_setup_window(config_manager: ConfigManager):
    """ SetupWindow 인스턴스를 생성하고 반환하는 유일한 통로 """
    ...
    class SetupWindow(FluentWindow):
        ...
        #
        # --- 여기에 이전에 작성했던 SetupWindow의 모든 메서드를 그대로 붙여넣습니다 ---
        # _create_widgets, _setup_layout, _connect_signals, load_settings, save_settings 등
        #
    ...
    return SetupWindow(config_manager)
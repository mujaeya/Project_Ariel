# src/gui/setup_window.py (새로운 최종 버전)

from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                             QLineEdit, QPushButton, QComboBox, QFormLayout,
                             QMessageBox, QFileDialog, QSpinBox, QKeySequenceEdit,
                             QCheckBox, QFrame, QSizePolicy, QScrollArea,
                             QGraphicsOpacityEffect)
from PySide6.QtCore import Qt, Signal
from PySide6.QtGui import QFont, QKeySequence
# [수정] qfluentwidgets에서는 FluentWindow만 가져옵니다.
from qfluentwidgets import FluentWindow
# [수정] TitleBar는 qframelesswindow에서 직접 가져옵니다.
...
# --- 이하 코드는 이전 답변과 동일하게 유지됩니다 ---
# SUPPORTED_LANGUAGES, CODE_TO_NAME, TranslationItem, OverlayPreview 클래스 및
# SetupWindow 클래스의 전체 내용은 변경할 필요가 없습니다.
# ... (이전 답변의 나머지 코드 전체) ...
SUPPORTED_LANGUAGES = {
...
class SetupWindow(FluentWindow):
    ...
    def __init__(self, config_manager: ConfigManager):
        print("DEBUG: SetupWindow.__init__ 시작")
        print("DEBUG: super().__init__() 호출 직전")
        super().__init__()
        print("DEBUG: super().__init__() 호출 완료")
        
        self.config_manager = config_manager
        print("DEBUG: setTitleBar() 호출 직전")
        self.setTitleBar(TitleBar(self))
        print("DEBUG: setTitleBar() 호출 완료")
        ...
        print("DEBUG: _create_widgets() 호출 직전")
        self._create_widgets()
        print("DEBUG: _create_widgets() 호출 완료")
        
        print("DEBUG: _setup_layout() 호출 직전")
        self._setup_layout()
        print("DEBUG: _setup_layout() 호출 완료")
        
        print("DEBUG: _connect_signals() 호출 직전")
        self._connect_signals()
        print("DEBUG: _connect_signals() 호출 완료")
        
        print("DEBUG: load_settings() 호출 직전")
        self.load_settings()
        print("DEBUG: load_settings() 호출 완료")
        
        print("DEBUG: SetupWindow.__init__ 완료")
    ...
    def save_settings(self):
        ...
        if deepl is not None and deepl_key_to_check:
            try:
